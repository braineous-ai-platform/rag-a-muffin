...
public final class CgoQueryPipeline implements QueryPipeline {
    ...
    public <T extends QueryTask> QueryExecution<T> execute(QueryRequest<T> request) {
        ...
        if (this.intelligenceBridge != null) {
            // Preserve the execution identity previously generated by LlmClientOrchestrator.
            request.generateRequestId();
            rawResponse = this.intelligenceBridge.invoke(llmPayload.toString(), "{}");
        } else {
            // Legacy llm-adapter path retained until the Arc path passes full regression.
            LlmClient client = this.findLlmClient();
            rawResponse = client.executePrompt(adapter, request, llmPayload);
        }
