 ...
 * // Goal: facts -> rules -> reasoning_path -> closure ("that's what it's all
 * about")
 * 
 ...
 * {}
 * 
 * record ReasoningPath(List<Fact> nodes, List<Edge> edges, double confidence,
 * String closure) {}
 * 
 * record Context(List<Fact> facts, Map<String,Object> state) {
 * Fact factById(String id) { return facts.stream().filter(f ->
 * f.id().equals(id)).findFirst().orElse(null); }
 * }
 * 
 * // ---------- Interfaces ----------
 ...
 * // Toy: split by parentheses or commas -> facts; tag with type hints if
 * present
 ...
 * // Example heuristics: performance ⇒ outcome; prep ⇒ confidence; friends ⇒
 * joy
 ...
 * (cx) -> new Fact("F_outcome","win / trophy / positive outcome",
 * Map.of("derivedBy","R1"))
 * );
 * 
 ...
 * (cx) -> new Fact("F_joy","fun evening with friends",
 * Map.of("derivedBy","R2"))
 * );
 * 
 * Rule synthesis = new Rule(
 * "R3","synthesis->day_quality",1.0,
 * lhsIdsByContains(facts,
 * "game","win","trophy","evening","fun","performance","prep"),
 * (cx) -> new Fact("F_day","A great accomplished day.",
 * Map.of("derivedBy","R3"))
 * );
 * 
 ...
 * }
 * 
 * public List<Edge> applyRules(Context cx, List<Rule> rules, Map<String,Object>
 * cfg) {
 ...
 * (List<Fact>)cx.state().computeIfAbsent("derivedFacts", k -> new
 * ArrayList<Fact>());
 ...
 * public ReasoningPath build(Context cx, List<Edge> edges, Map<String,Object>
 * cfg) {
 * // Build path: start from earliest prompt facts, walk highest-scoring edges
 * to derived conclusions
 * @SuppressWarnings("unchecked")
 * List<Fact> derivedFacts = (List<Fact>)cx.state().getOrDefault("derivedFacts",
 * List.of());
 ...
 * Set<String> hasOut = edges.stream().map(e ->
 * e.fromFactId()).collect(Collectors.toSet());
 * Set<String> hasIn = edges.stream().map(e ->
 * e.toFactId()).collect(Collectors.toSet());
 * List<Fact> terminals = hasIn.stream().filter(id ->
 * !hasOut.contains(id)).map(all::get).filter(Objects::nonNull).toList();
 * 
 * // assemble a simple linearized path by sorting edges by score then sequence
 * List<Edge> sorted = edges.stream().sorted((a,b)->Double.compare(b.score(),
 * a.score())).toList();
 * LinkedHashSet<String> order = new LinkedHashSet<>();
 * for (Edge e : sorted) { order.add(e.fromFactId()); order.add(e.toFactId()); }
 * List<Fact> pathFacts =
 * order.stream().map(all::get).filter(Objects::nonNull).toList();
 * 
 * double confidence =
 * sorted.stream().mapToDouble(Edge::score).average().orElse(0.0);
 ...
 * Optional<Fact> day = path.nodes().stream().filter(f ->
 * f.text().toLowerCase().contains("great accomplished day")).findFirst();
 * if (day.isPresent()) return day.get().text();
 * String joined =
 * path.nodes().stream().map(Fact::text).collect(Collectors.joining(" → "));
 ...
 * boolean unitTest = Boolean.TRUE.equals(cfg.getOrDefault("unit_test_mode",
 * false));
 * Map<String,Object> state = new HashMap<>(Map.of("seed",
 * cfg.getOrDefault("seed", 42)));
 * 
 ...
 * 
 * if (unitTest) {
 * Unit.assertContains(summary, "great accomplished day", "Summary should
 * conclude with the day-quality statement");
 * Unit.assertEquals(path.closure(), summarizer.closurePhrase(), "Closure phrase
 * must match signature");
 * Unit.assertTrue(path.confidence() >= 0.6, "Confidence must meet minimum
 * threshold");
 * }
 ...
 * 
 * // ---------- DTOs ----------
 * public record Output(List<Fact> facts, List<Rule> rules, List<Edge> edges,
 * ReasoningPath path, String summary) {}
 * 
 * // ---------- Tiny Unit Helper ----------
 * static class Unit {
 * static void assertTrue(boolean cond, String msg){ if(!cond) throw new
 * AssertionError(msg); }
 * static void assertContains(String hay, String needle, String msg){ if
 * (hay==null || !hay.toLowerCase().contains(needle)) throw new
 * AssertionError(msg + " [needle="+needle+", got="+hay+"]"); }
 * static void assertEquals(Object a, Object b, String msg){ if
 * (!Objects.equals(a,b)) throw new AssertionError(msg + " [a="+a+", b="+b+"]");
 * }
 * }
 * 
 * // ---------- Example Usage ----------
 * public static void main(String[] args) {
 * String prompt = "(that excellent cricket game) -> (amazing
 * performance/effort/prep) -> (win, trophy, fun evening of street cricket with
 * friends)";
